...
namespace Alareqi\FilamentExtend\Commands\Concerns;
...
{
    protected function getTransaltionPrefix(string $model): string
    {
        return Str($model)
            ->afterLast('\\')
            ->singular()->snake();
    ...
    protected function getResourceTableColumns(string $model): string
    {
        ...
        $translationPrefix = $this->getTransaltionPrefix($model);
        ...
        foreach ($table->getColumns() as $column) {
            ...
            if ($modelObject->translatable) {
                $isTranslatable = in_array($columnName, $modelObject->translatable);
            }
            if (
                $column->getType()::class ==
                Types\TextType::class && !$isTranslatable
            ) {
                continue;
            }

            if ($isTranslatable) {
                $columnData['label'] = ['fn ($livewire): string => __(\'' . $translationPrefix . '.fields.' . $column->getName() . '.label\') . \' [\' . __(\'common.languages.\' . $livewire->activeLocale) . \']\''];
            } else {
            ...
            if ($column->getType() instanceof Types\BooleanType) {
                ...
                $columnData['type'] = match (true) {
                    $columnName === 'image', str($columnName)->startsWith('image_'), str($columnName)->contains('_image_'), str($columnName)->endsWith('_image') => Tables\Columns\ImageColumn::class,
                        // $column->getType() instanceof Types\TextType && $isTranslatable => Tables\Columns\TextColumn::class,
