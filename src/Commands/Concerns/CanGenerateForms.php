    ...
    protected function getResourceFormSchema(string $model): string
    {
        ...
        foreach ($table->getColumns() as $column) {
            ...
            $componentData['type'] = match (true) {
                ...
                $column->getType()::class === Types\TextType::class && $isTranslatable => Forms\Components\TextInput::class,
                $column->getType()::class === Types\TextType::class && !$isTranslatable => Forms\Components\Textarea::class,
            ...
            if ($isTranslatable) {
                $componentData['label'] = ['fn ($livewire): string => __(\'' . $translationPrefix . '.fields.' . $columnName . '.label\') . \' [\' . __(\'common.languages.\' . $livewire->activeLocale) . \']\''];
            } else {
